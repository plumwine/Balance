...
void GamePlayManager::Initialize()
{
	...
	endGr    = GraphFactory::Instance().LoadGraph("../Texture/master/sipai_A.png");      //失敗仮
	golGr    = GraphFactory::Instance().LoadGraph("../Texture/master/seikou_A.png");     //成功仮
	back_Gr = GraphFactory::Instance().LoadGraph("../Texture/master/Haikei.png");        //背景
	numberGr = GraphFactory::Instance().LoadGraph("../Texture/master/Renban.png");       //数字
...
void GamePlayManager::GameUpdate(float deltaTime)
{
	...
	Render::Instance().RectParticle(Vector2(0, 900), enemyDeadCount * 3, 192, false); //ゲージ
	Render::Instance().RectParticle(Vector2(0, 900), cannonCount, 64, false); //ゲージ
	CountMnager();
}
